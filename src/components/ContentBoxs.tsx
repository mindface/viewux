import { Form, Button, Input, Header, Select, Popup } from 'semantic-ui-react'
...
function ContentBoxtooler() {
  ...
  const downloadAction = () => {
    ...
    helperFile.downLoadJSON<{ verticalList: BoxList; sideList: BoxList }>(
      jsonItem,
      'verticalandside.json',
    )
  }
  ...
  const textCopy = () => {
    ...
    sideList.forEach((side) => {
      text += '------------------------------ \n'
      text += `${side.info} | ${side.type} \n`
      if (side.list && side.list?.length > 0) text += 'vertical ------------ \n'
      side.list?.forEach((vertical) => {
        text += `-- ${vertical.info}  \n`
        text += ` +- ${vertical.id} (id) \n`
      })
      if (side.list && side.list?.length > 0) text += '------------ \n'
    })
    text += '------------------------------ \n'
  ...
  return (
    <div className="content-boxs">
      ...
      <div className="math-actions"></div>
      <div className="v-boxs">
        <div className="box-lists">
          <div className="box-list">
            <div className="flex">
              <div className="box-item">横</div>
              {verticalViewList.map((vertical, k) => (
                <div key={'vertical' + k} className="box-item">
                  {vertical?.info} |{' '}
                  <span className="tag">{viewTag(vertical.tag ?? '')}</span>
                  <div className="actions">
                    <Button onClick={() => deleteVerticalAction(vertical.id)}>
                      削除
                    </Button>
                  </div>
                </div>
          ...
          <div className="box-list flex">
            ...
            {sideList.map((side, k) => (
              <div key={'side' + side.id + k} className="box-item">
                <Header as="h5">{side.info}</Header>
                <div className="actions">
                  <Button onClick={() => deleteSideAction(side.id)}>
                    削除
                  </Button>
                </div>
              </div>
      ...
      <div className="blocks-box">
        <div className="box-list">
          ...
          <div className="side-list">
            {sideList.map((side, k) => (
              <div key={`sideView${k}`} className="side-item flex">
                <div className="side-info">
                  <div>{side.info}</div>
                </div>
                <div className="vertical-list flex">
                  {reSideList(side?.list ?? []).map((vertical: Box, bk) => (
                    <div
                      key={`sideListVerticalView${bk}`}
      ...
      <div className="blocks-box p-1">
        ...
        <Popup
          content="対象は縦と横のタイトル"
          trigger={<Button onClick={() => textCopy()}>テキストコピー</Button>}
        />
      </div>
    </div>
  )
