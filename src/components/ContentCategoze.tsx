...
function ContentCategoze() {
  ...
  const setFileListCategozeAction = (
    item: FileItem,
    infoList: FileCategozeList,
  ) => {
    const list = fileList.map((file) => {
      if (file.name === item.name) {
        ...
        infoList.forEach((info, index) => {
          text +=
            infoList.length === index ? info.className + '| ' : info.className
        })
    ...
    <div className="content-categoze">
      <div className="p-1">
        ...
        <div className="file-output flex">
          {fileList.map((file, k) => (
            <div className="file-info" key={`file${k}`}>
              {file.name} |{' '}
              <Button
                onClick={() => {
                  FileCategozeAction(file)
                }}
              >
                categoraize
              </Button>
